...
3. `tasks.md` — checklist de pasos discretos y verificables, con dependencias y
   ...
   **Incluye siempre una seccion `## Archivos esperados`** con los archivos que la feature
   va a escribir, en globs a nivel de modulo (`lib/modules/proveedores/**`), no rutas
   exactas. El leader la copia al campo `files` de `feature_list.json` al pasar la ficha a
   `spec_ready`, y es lo que decide si esta feature puede correr en paralelo con las que ya
   estan en vuelo (`AGENTS.md > Paralelismo`). Una lista corta de mas bloquea trabajo que no
   choca; una corta de menos deja entrar dos features que se van a pisar.
