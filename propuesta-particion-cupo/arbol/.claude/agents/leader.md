...
- **Lo que limita el paralelismo son los archivos, no la zona.** Dos features corren a la vez
  si su `files` no intersecta, sin importar `zone`, descontando los compartidos por convencion
  (`db/schema.prisma`, `feature_list.json`, `progress/**`). Encima hay un **tope global de 5**
  `in_progress`. Lo valida `./init.sh` (`AGENTS.md > Paralelismo`).
- **Una ficha puede traer mas de un trabajo dentro.** Los ejes de corte y cuando aplica cada uno
  estan en `AGENTS.md > Criterios de particion`. Si la ficha no paso por `/afinar-feature`, ese
  filtro no corrio: miralo tu en F1.0. `zone: fullstack` es estado de transito y el gate falla si
  sobrevive hasta `spec_ready` o `in_progress`.
...
3. Monta el worktree de la feature con `./scripts/wt.sh new <key> <slug>` (crea la rama
   ...
   `git checkout` en el.
4. Delega en `spec_author` con el modelo segun complexity. Cuando termine, **copia los
   archivos esperados de `tasks.md` al campo `files`** (sustituye la version gruesa de F1.0),
   cambia la feature a `spec_ready`, **mueve la tarjeta a *Spec en revision*** con un comentario
