...
| `files` | **estado de disco, no del board**: los archivos que la feature va a escribir. Lo derivan `/afinar-feature` (Paso 1b) y el leader (F1.0 grueso, F1.3 fino desde `tasks.md`). No viaja a Jira: un humano no declara archivos previstos. |
...
3. **`files` no se toca nunca desde el board**, porque no vive allí: lo deriva el arnés
   leyendo el spec. F0 lo conserva tal cual.
...
| la ficha resulta traer más de un trabajo dentro y se parte (Paso 1b) | se **crean** las fichas de la partición, encadenadas con «is blocked by» y con `Archivos previstos:` en la Description. La original conserva el primer trozo y su `description` se reescribe; **se siembra solo ella** |
