...
1. **Lo que limita el paralelismo son los archivos, no la zona.** Dos features pueden estar
   `in_progress` a la vez si **no escriben en los mismos archivos**, sin importar su `zone`.
   Cada ficha declara dónde escribe en el campo `files` de `feature_list.json`, y `./init.sh`
   cruza los de todas las `in_progress` (`scripts/validate-features.mjs`). Sobre eso hay un
   **tope global de 5 `in_progress`** como red de seguridad: los costes de tener muchas en
   vuelo —la cola de merges hacia `dev`, el tiempo de gate, la revisión humana— no se ven en
   un cruce de archivos. Detalle y lista de archivos compartidos por convención en
   `AGENTS.md > Paralelismo`. La regla también se puede violar arrastrando tarjetas en el
   board: si el board la incumple, **gana la regla** — el leader deja fuera la feature
   sobrante al importar y lo dice.
