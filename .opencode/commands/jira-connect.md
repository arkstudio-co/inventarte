...
**Ojo con el prefijo de las tools de Jira.** En opencode las tools de un servidor MCP se registran
con el nombre del server por delante: las de `atlassian` aparecen como `atlassian_*`. No busques
`getVisibleJiraProjects` ni `atlassianUserInfo` por nombre pelado: buscalas por el prefijo
`atlassian_` (y por el sufijo reconocible). Si no ves ninguna con el prefijo `atlassian_`, la
conexion no esta montada.

## Regla que no se rompe: el token no entra en el chat

**Nunca pidas que peguen el token ni el `ATLASSIAN_MCP_AUTH` en la conversacion**, ni ejecutando
comandos con el prefijo de ejecucion local de la interfaz (`!`). Todo lo que pasa por aqui queda
en el transcript, en el historial de la sesion y en los logs. El comando **dicta** el comando a
ejecutar; la persona lo corre en **otra terminal** y solo vuelve a decir "hecho". Si aun asi lo
pegan, dilo, y di que ese token hay que revocarlo y generar otro — no sigas como si nada.

## Paso 1 — Detectar en que fase estas

Llama a la tool de Jira que lista los proyectos visibles (prefijo `atlassian_`). El resultado
decide todo lo que sigue:

- **No existe ninguna tool `atlassian_*`** en tu set → no hay conexion. Ve a la **Fase A**.
- **La tool responde** → hay conexion. Ve a la **Fase B**.
- **La tool existe pero da error** → hay credencial pero es incorrecta. Diagnostica con la tabla
  del Paso 4 y luego ve a la Fase A por el paso que falle.

No uses el indicador de MCP de la interfaz ni el color del servidor para decidir: el servidor de
Atlassian conecta **sin credencial valida** y expone igual tres herramientas de Teamwork Graph. Si
solo ves esas tres, no hay conexion util por mucho que el indicador este en verde.
...
5. Ponerle un nombre reconocible (p. ej. `opencode-<maquina>`), para poder revocar **solo ese**
...
terminal aparte, no con el prefijo de ejecucion local.
...
opencode. Que verifique en la terminal nueva, antes de arrancar, imprimiendo la longitud de
`$env:ATLASSIAN_MCP_AUTH` — un numero esta bien; vacio o error significa que A.3 no llego.

**El comando termina aqui**: la conexion MCP se establece al arrancar opencode, asi que la
...
**B.1 — Quien eres.** Llama a la tool de identidad de Jira (prefijo `atlassian_`) y di con que
cuenta esta entrando. Sirve para cazar el error mas silencioso de todos: estar conectado con la
cuenta de otro —por un token compartido o uno viejo en la variable— y no enterarse hasta que el
board muestre a la persona equivocada aprobando specs.

**B.2 — Los proyectos a los que tiene acceso.** Lee `jira.project` de `feature_list.json`:
ese es el proyecto que este repo espera, no uno que tengas memorizado. Con el resultado de la
tool que lista proyectos, lista cada proyecto con **clave y nombre** y marca explicitamente
...
**B.4 — Cierre.** Una linea con: cuenta, si el proyecto declarado esta, y cual es el siguiente
paso real — el arranque de sesion del agente `leader`, que empieza importando el board (F0 de
`AGENTS.md`).
